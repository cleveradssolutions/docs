...
```ts
// Not available in React Native API.
// Store the user ID on your backend or pass network-specific IDs via `mediationExtras` at initialize().
````
...
```ts
import { CASMobileAds, Gender } from 'react-native-cas';
...
```ts
// Direct latitude/longitude setter is not available in React Native.
// Use automatic (coarse) location collection if permitted (see below).
```
