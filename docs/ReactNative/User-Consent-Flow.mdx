...
<Info>
For Expo projects, add `NSUserTrackingUsageDescription` in `app.json` (or `app.config.js/ts`) under `expo.ios.infoPlist`, then run `npx expo prebuild` and rebuild the iOS app.
</Info>
...
## Manual consent flow
Call `CASMobileAds.showConsentFlow()` to show the built-in consent form manually.  If the consent is required, the SDK loads the form and immediately presents it.  
...
```ts
import CASMobileAds, { ConsentFlowStatus } from 'react-native-cas';

CASMobileAds.showConsentFlow().then((status) => {
...
| STILL_PRESENTING       | There was an error with another form is still being displayed.     |
...
function initialize() {
  CASMobileAds.initialize(casId, {
    ...
    isConsentObtained = status.consentFlowStatus === ConsentFlowStatus.OBTAINED; // [!code highlight]
  });
}

function showPrivacyOptionsForm() {
  CASMobileAds.showConsentFlow().then((status) => {
    isConsentObtained = status === ConsentFlowStatus.OBTAINED;
  });
}
```
