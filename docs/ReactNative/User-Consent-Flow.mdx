...
The `Future<int>` completes after the form is dismissed. If consent is not required, the `Future` completes immediately.
...
## Debug geography
The SDK provides a way to test your app's behavior as though the device was located in the EEA or UK using the [PrivacyGeography](https://pub.dev/documentation/clever_ads_solutions/latest/clever_ads_solutions/PrivacyGeography.html) enum. 
...
CASMobileAds.initialize(  
  ...
  debugPrivacyGeography: PrivacyGeography.europeanEconomicArea, // [!code highlight]
)
```
...
```dart
ConsentFlow.create(  
  debugGeography: kDebugMode // [!code highlight]
      ? PrivacyGeography.europeanEconomicArea // [!code highlight]
