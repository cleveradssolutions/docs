...
useEffect(() => {
  (async () => {
    try {
      await InterstitialAd.setAutoloadEnabled(true);
...
useEffect(() => {
  const offLoaded = InterstitialAd.addAdLoadedEventListener(() => console.log('Loaded'));
  const offLoadFailed = InterstitialAd.addAdLoadFailedEventListener((err) => console.log('Load failed:', err));
  const offShowed = InterstitialAd.addAdShowedEventListener(() => console.log('Showed'));
  const offFailedToShow = InterstitialAd.addAdFailedToShowEventListener((err) => console.log('Failed to show:', err));
  const offClicked = InterstitialAd.addAdClickedEventListener(() => console.log('Clicked'));
  const offDismissed = InterstitialAd.addAdDismissedEventListener(() => console.log('Dismissed'));
  const offImpression = InterstitialAd.addAdImpressionEventListener((imp) => console.log('Impression', imp));
...
```ts
const isLoaded = await InterstitialAd.isAdLoaded();
if (isLoaded) await InterstitialAd.showAd();
```
...
await InterstitialAd.setAutoshowEnabled(true); // [!code highlight]
await InterstitialAd.loadAd();
```
...
```ts
import { InterstitialAd } from 'react-native-cas';
await InterstitialAd.setMinInterval(0); // [!code highlight]
```
