...
```ts
useEffect(() => {
  const unsubscribeLoaded = AppOpenAd.addAdLoadedEventListener(() => console.log('AppOpen loaded'));
  const unsubscribeLoadFailed = AppOpenAd.addAdLoadFailedEventListener((err) => console.log('Load failed:', err));
  const unsubscribeShowed = AppOpenAd.addAdShowedEventListener(() => console.log('Ad showed'));
  const unsubscribeFailedToShow = AppOpenAd.addAdFailedToShowEventListener((err) => console.log('Failed to show:', err));
  const unsubscribeDismissed = AppOpenAd.addAdDismissedEventListener(() => console.log('Ad dismissed'));
  const unsubscribeClicked = AppOpenAd.addAdClickedEventListener(() => console.log('Ad clicked'));
  const unsubscribeImpression = AppOpenAd.addAdImpressionEventListener((imp) => console.log('Ad impression', imp));
...
```ts
const isAdLoaded = AppOpenAd.isAdLoaded();
```
...
// Рекомендовано разом ввімкнути автозавантаження, щоб не викликати loadAd() вручну
AppOpenAd.setAutoloadEnabled(true);
```
