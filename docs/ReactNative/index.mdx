...
CAS.AI uses native modules, so it is not supported in **Expo Go**. Use a development build instead.
...
Run your app with Expo native builds:
...
For Expo projects, CAS can be configured through `app.json` / `app.config.js` using the Expo config plugin (see below). 
</TabItem> 
</Tabs>
...
If you use Expo (development build / prebuild workflow), you can configure CAS via `app.json` / `app.config.js`.
The config plugin will patch the generated native projects during `expo prebuild`.
...
Enable one or more solution packs:

- `"optimal"` — Optimal Ads Solution
- `"families"` — Families Ads Solution (Advertising ID / IDFA will be disabled automatically)
...
Enable any number of adapters. This can be used together with `solutions`.
  ...
  "expo": {
    "plugins": [
      [
        ...
        {
          "solutions": ["optimal"],
          "adapters": ["unityAds", "pangle", "mintegral"]
        }
      ]
    ]
  }
...
After updating `solutions` / `adapters`, regenerate native projects:
...
<Info>
The sections below describe the **manual** setup for Android and iOS (non-Expo workflow or custom native projects).
</Info>
...
If you haven't created an CAS account and registered an app yet, now's a great time to do so at [https://cas.ai](https://cas.ai)
...
For Expo projects, use `adapters` to enable any number of network adapters:
...
If you enable the Families solution, `useAdvertisingId` will be disabled automatically. </TabItem>
...
For Expo projects, set this key using the config plugin option `trackingDescription`, then run `npx expo prebuild` and rebuild iOS.
