...
After `expo prebuild`, complete the platform setup below in the generated `android` and `ios` projects.
</TabItem>
</Tabs>
...
<Info>
For Expo projects, apply the Android and iOS steps below in the generated native folders after running `npx expo prebuild`.
</Info>
...
For Expo projects, set this key in `app.json` (or `app.config.js/ts`) under `expo.ios.infoPlist`, then run `npx expo prebuild` and rebuild iOS.
