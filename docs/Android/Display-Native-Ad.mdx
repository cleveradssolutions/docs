...
Values for registered views will be set automatically when calling `bindAdContent()`.  
...
<Warning>  
The `bindAdContent(NativeAdContent)` method must be called from the main (UI) thread.
</Warning>
...
fun NativeAdTemplateView(adContent: NativeAdContent, modifier: Modifier = Modifier) {
    ...
    LaunchedEffect(adContent, nativeView) {
        nativeView.bindAdContent(adContent)
    }
