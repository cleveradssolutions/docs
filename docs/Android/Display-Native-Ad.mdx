...
You can display the built-in native template inside Compose using `CASNativeView` + `AndroidView`.

```kotlin
@SuppressLint("ConfigurationScreenWidthHeight")
@Composable
fun NativeTemplateAd(modifier: Modifier = Modifier) {
    val context = LocalContext.current
    val screenWidthDp = LocalConfiguration.current.screenWidthDp

    // If you render the view with horizontal padding = 16dp, align template width with available space.
    ...
    val nativeView = remember(availableWidthDp) {
        CASNativeView(context).apply {
            setAdTemplateSize(AdSize.getInlineBanner(availableWidthDp, 300))
        }
    }
    ...
    DisposableEffect(Unit) {
        ...
        CASNativeLoader(context, MyApplication.CAS_ID, callback).apply {
...
- [Compose Native Ad View (manual)](https://github.com/cleveradssolutions/CAS-Android/blob/master/composeSample/src/main/java/com/cleveradssolutions/sampleapp/formats/NativeScreen.kt)
- [Compose Native Ad Template View](https://github.com/cleveradssolutions/CAS-Android/blob/master/composeSample/src/main/java/com/cleveradssolutions/sampleapp/formats/NativeTemplateScreen.kt)
