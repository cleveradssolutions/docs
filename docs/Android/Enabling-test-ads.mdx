...
- [Enable test ad mode](Android/Enabling-test-ads#enable-test-ad-mode) for all devices.
- [Set device ids to test ad](Android/Enabling-test-ads#enable-test-devices).
...
<CodeGroup synchronize={true}>
```kotlin
CAS.buildManager().withTestAdMode(true).initialize()
```
...
<CodeGroup synchronize={true}>
```kotlin
CAS.settings.testDeviceIDs = setOf("12345678901234567890123456789012")
```

```java
CAS.settings.testDeviceIDs = new HashSet<>(Arrays.asList("12345678901234567890123456789012"));
```
</CodeGroup>
