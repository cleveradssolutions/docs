...
- For native video ads, the main asset `mediaView` must be at least 120x120dp. Video ads won't serve to implementations with main asset `CASMediaView` smaller than 120dp in any dimension.
...
```kotlin
adLoader.adChoicesPlacement = AdChoicesPlacement.TOP_RIGHT
```
...
<CodeGroup synchronize={true}>
```kotlin
val starRatingView = adView.findViewById<CASStarRatingView>(R.id.ad_star_rating)
...
<CodeGroup synchronize={true}>
```kotlin
class CustomRatingView @JvmOverloads constructor(context: Context,attrs: AttributeSet? = null) : View(context, attrs), NativeAdStarRating {
    override fun getRating(): Double? = 5.0
    override fun setRating(value: Double?) {
        // Update star rating view
    }
}

adView.starRatingView = adView.findViewById<CustomRatingView>(R.id.ad_star_rating)
```
...
<CodeGroup synchronize={true}>
```kotlin
val customRatingView = adView.findViewById<RatingBar>(R.id.ad_star_rating)
customRatingView.numStars = 5
customRatingView.stepSize = 0.5f

nativeAd.starRating?.let { rating ->
    customRatingView.rating = rating.toDouble()
}
