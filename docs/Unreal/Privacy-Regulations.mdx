...
```cpp
UCASMobileAds::SetUserAudienceForAds(ECASAudience::Children);
```
...
### Not Children Audience
If you know that the user does not fall within an age-restricted category, you must set the `NotChildren` audience flag:
```cpp
UCASMobileAds::SetUserAudienceForAds(ECASAudience::NotChildren);
```
<Image width="296" alt="image" src="/assets/unreal/Privacy-Regulations-1.png" />

### Undefined Audience
The user's age has not been determined.
```cpp
UCASMobileAds::SetUserAudienceForAds(ECASAudience::Undefined);
```
