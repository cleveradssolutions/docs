...
## iOS Crash on app launch
If you're getting a crash on app launch with the following symbols, it means the CAS Unreal Plugin failed to add the Google App ID to your app's Info.plist for some reason. 
```
0x0000000103348aac GADForwardedUserDefaults + 484
0x0000000103330270 GADApplicationVerifyPublisherInitializedCorrectly + 2660
```

You should share the build details of your app with your account manager. If the automation issue can't be resolved, the manager will provide an identifier that you'll need to add to your `Config/DefaultEngine.ini` file in the following format.
```ini
