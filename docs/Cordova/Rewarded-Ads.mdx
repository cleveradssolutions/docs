...
When you want to manually control ad loading, use the `load()` function.
...
```js
// Using the same onAdLoaded / onAdLoadFailed handlers as above.
casai.rewardedAd.load({
  autoReload: false,
});
```
...
```js
function onAdReward(e) {
  if (e.detail.format === casai.Format.REWARDED) {
    // Called when the user has earned the reward.
    console.log('Reward the user for watching the ad', e.detail);
  }
}
...
CAS provides the option to load interstitial ads as a fallback when a rewarded video ad has no available fill.
Interstitial ads do not require the user to watch the entire ad to completion.
However, the reward will still be granted as if the user completed the rewarded video.

```js
casai.rewardedAd.setExtraFillByInterstitialAd(true); // [!code highlight]
```
