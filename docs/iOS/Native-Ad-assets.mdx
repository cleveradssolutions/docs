...
- For native video ads, the main asset `mediaView` must be at least 120x120 points. Video ads won't serve to implementations with main asset `CASMediaView` smaller than 120 points in any dimension.
...
The `CASMediaView` is a special `UIView` designed to display the main media asset, either video or image. Can be defined in an XML layout or constructed dynamically. It should be placed within the view hierarchy of a `CASNativeView`, just like any other asset view.  The media view will be populated automatically when calling `bindAdContent()`.
