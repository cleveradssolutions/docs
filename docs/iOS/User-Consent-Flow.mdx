...
flow.viewControllerToPresent = self;
flow.completionHandler = ^(CASConsentFlowStatus status) {
    if (status == CASConsentFlowStatusObtained) {
        // User consent obtained.
    } else if (status == CASConsentFlowStatusNotRequired) {
        // Consent not required.
    } else {
        NSLog(@"Consent flow finished with status: %ld", (long)status);
    }
};
...
flow.viewControllerToPresent = self;
flow.completionHandler = ^(CASConsentFlowStatus status) {
    NSLog(@"Consent Flow finished: %ld", (long)status);
};
    
flow.debugGeography = CASUserDebugGeographyEEA;

#ifdef DEBUG
    flow.forceTesting = YES;
#endif
    [flow presentIfRequired];
