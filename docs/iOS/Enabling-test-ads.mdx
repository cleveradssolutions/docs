...
- [Enable test ad mode](iOS/Enabling-test-ads#enable-test-ad-mode) for all devices.
- [Set device ids to test ad](iOS/Enabling-test-ads#enable-test-devices).
...
<CodeGroup synchronize="true">
```swift
buildManager.withTestAdMode(true)
```

```objc
[buildManager withTestAdMode:YES];
```
</CodeGroup>
...
2. Check the console output for a message that looks like this:
```
...
<CodeGroup synchronize="true">
```swift
CAS.settings.setTestDevice(ids: ["12345678901234567890123456789012"])
```

```objc
[CAS.settings setTestDeviceWithIds:@[@"12345678901234567890123456789012"]];
```
</CodeGroup>
